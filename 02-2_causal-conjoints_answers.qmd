...
1. See code below.
2. **They're similar because the experimental design does most of the work.** Attribute levels are randomized across respondents and tasks. In large samples with balanced designs, the conditional logit and LPM should converge to the same average causal effects. They would diverge more noticeably with (1) small samples where the linear approximation to the logit curve breaks down near 0 or 1, (2) highly heterogeneous respondents whose baseline rates differ a lot from 0.5, or (3) designs with systematic imbalances in attribute level assignment. But we're all good here.
:::
...
all_mms_clogit <- attribute_lookup |>
  mutate(
    results = map(
      attribute,
      \(att) {
        avg_predictions(model_clogit, by = att) |>
...
ggplot(all_mms_clogit, aes(x = estimate, y = level_nice)) +
...
all_amces_clogit <- attribute_lookup |>
  mutate(
    results = map(
      attribute,
      \(att) avg_predictions(
  ...
  unnest(results) |> 
  select(-attribute_nice)

reference_rows <- all_amces_clogit |>
  separate_wider_delim(hypothesis, delim = " - ", names = c("level", "ref")) |>
  # Remove the opening and closing ()s that hypothesis adds to these things
  mutate(across(c(level, ref), \(x) str_sub(x, start = 2, end = -2))) |> 
  select(attribute, level = ref) |>
  distinct() |>
  mutate(estimate = 0, conf.low = 0, conf.high = 0)

plot_data <- all_amces_clogit |>
  separate_wider_delim(hypothesis, delim = " - ", names = c("level", "ref")) |>
  mutate(across(c(level, ref), \(x) str_sub(x, start = 2, end = -2))) |> 
  bind_rows(reference_rows) |> 
  left_join(attribute_levels_lookup, by = join_by(attribute, level))
...
1. See code below.
2. Bayesian probability of direction is nice and straightforward: "there is a 90.7% chance that 65-year-old candidates are less preferred than 29-year-olds, holding all other attributes constant." In frequentist statistics, assuming this p-value is something like 0.1ish, we'd say "in a world where there's no difference between preferences for 65-year-old and 29-year-old candidates, we do not have evidence to reject the null hypothesis and thus do not know if there's a significant difference between them"
3. See code below.
4. See code below.
:::
...
# Age plots
p1 <- preds_age_marginalized |> 
  ggplot(aes(x = avg, y = age, fill = age)) + 
  stat_halfeye() + 
  geom_vline(xintercept = 0.5) + 
  scale_fill_manual(values = clrs[c(1, 3, 5)], guide = "none") + 
  labs(title = "Marginal means", y = NULL, x = "Marginal mean")

reference_row <- tibble(level = "29", avg = 0)

p2 <- preds_age_marginalized |>
  compare_levels(variable = avg, by = age, comparison = "control") |> 
  separate_wider_delim(age, delim = " - ", names = c("level", "ref")) |> 
  bind_rows(reference_row) |> 
  left_join(attribute_levels_lookup, by = join_by(level)) |> 
  ggplot(aes(x = avg, y = level_nice, fill = level_nice)) + 
  stat_halfeye() + 
  ...
  scale_x_continuous(labels = label_pp) + 
  scale_fill_manual(values = clrs[c(1, 3, 5)], guide = "none") + 
  labs(title = "AMCEs", y = NULL, x = "AMCE")

p1 | p2

# Everything at once
all_things <- attribute_lookup |> 
  mutate(mms = map(attribute, \(att) {
    all_preds |>
      summarize(avg = mean(.epred), .by = c(all_of(att), .draw)) |> 
      # Make sure the attribute column is named "level"
      rename_with(~"level", all_of(att))
  })) |> 
  mutate(amces = map(mms, \(mms) {
    mms |>
      compare_levels(variable = avg, by = level, comparison = "control")
  })) |> 
  mutate(p_lt_0 = map(amces, \(x) {
    x |> 
      summarize(probability_lt_0 = sum(avg < 0) / n())
  }))
...
all_things |> 
  ...
  ggplot(aes(x = avg, y = level_nice, fill = attribute_nice)) + 
  stat_halfeye() +
  geom_vline(xintercept = 0.5, linetype = "dashed", color = "grey50") +
  scale_x_continuous(labels = label_percent()) +
  scale_fill_manual(values = clrs[1:6], guide = "none") +
  labs(
    x = "Marginal mean",
    y = NULL
  ) +
...
reference_rows <- all_things |> 
  unnest(amces) |> 
  separate_wider_delim(level, delim = " - ", names = c("level", "ref")) |>
  select(attribute, level = ref) |>
  distinct() |>
  left_join(attribute_levels_lookup, by = join_by(attribute, level)) |> 
  select(-level_nice) |> 
...
all_things |>
  unnest(p_lt_0) |> 
  select(attribute_nice, level, probability_lt_0)
```
...
1. See code below. Both Democrats and Republicans show a positive and increasing effect of years in politics---more experience is better for both groups.
2. See code below.
3. There's a difference between the parties! The AMCE tends to be larger for Democrats than for Republicans.
:::
...
# Marginal means
preds_years_party <- all_preds_hier |>
  group_by(years_politics, resp_party, .draw) |>
  summarize(avg = mean(.epred))

# Differences in marginal means
amces_years_party <- preds_years_party |>
  group_by(resp_party) |>
  compare_levels(variable = avg, by = years_politics, comparison = "control")

amces_years_party |>
  group_by(resp_party, years_politics) |>
  median_qi(avg)
...
amces_years_party |>
  ...
  scale_x_continuous(labels = label_pp) +
  scale_fill_manual(values = party_colors, guide = "none") +
  facet_wrap(vars(years_politics), ncol = 1) +
  labs(
    x = "AMCE (vs. no experience)",
    y = "Respondent party",
    title = "Posterior conditional AMCEs for candidate experience"
  )
```
...
The multilevel model diverges from pooled OLS most when (1) there is strong respondent-level heterogeneity in responses to specific attributes---that is, when different respondents weight the same features very differently; (2) the sample has substantial imbalance across respondents (some people completing many more tasks than others), because partial pooling adjusts more aggressively for respondents with fewer observations; and (3) the experiment uses a small number of respondents but a large number of tasks per respondent, where respondent-specific effects are the dominant source of variation. In this candidate experiment, with ~2,100 respondents doing only 3 tasks each, there is not enough within-person data for the random intercepts to shift the population-average AMCEs meaningfully.
:::
...
::: {.callout-note title="Answer"}
With a balanced randomized design and a large sample, the OLS estimate is consistent regardless of the likelihood. The AMCE is identified by the design, not the model. So if "what's the average effect of candidate gender?" is your only estimand, pooled OLS with clustered SEs is defensible and you technically don't need Bayes.

But you get all these things!

1. **Heterogeneous effects across respondent characteristics**. You can ask: "is the gender AMCE larger among Democrats than Republicans?" and get a full posterior distribution of that difference, not just a point estimate. You can calculate all sorts of predictions and comparisons with {marginaleffects} with OLS, but you lose proper uncertainty propagation when you start computing differences-of-differences-of-predictions.

2. **Probabilistic claims about effects**. It's so nice to say "there is a 73% chance the Republican gender AMCE is negative" instead of accepting or rejecting p-values.

3. **Individual-level utilities.** The random effects give partial estimates of each respondent's preferences. This is what commercial conjoint tools (Sawtooth, OpinionX) are doing, and we'll see more of it tomorrow.

It took @JensenMarbleScheve2021 days to run their models back in 2021. Nowadays, Stan is a lot faster. You might as well do it 🤷‍♂️. Also, if you don't want the *full* full model, you can omit the respondent-level characteristics and just focus on regular AMCEs and marginal means with Bayesian methods (that's what we do in @ChaudhryDotsonHeiss2025), and it goes really quick---just a couple minutes per model.
:::
